...
在 [job-filler](https://github.com/freecodetiger/job-filler) 基础上二次开发：保留「上传简历 PDF → AI 解析 → 一键填写网申表单」的核心能力，并新增**求职投递追踪**，把单纯的「填写历史」升级为「投递历史」，让秋招海投不再乱。
...
1. **配置 API** — 点击插件图标 → 「API」标签 → 填入：
   ...
   - 点击「保存」
2. **导入简历** — 「导入」标签 → 上传 PDF 简历 → AI 自动解析 → 确认导入
